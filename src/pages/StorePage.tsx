...
const avnerHighlights = [
  {
    icon: Palette,
    body: "אבנר איש אשכולות אמיתי המשלב רקע אקדמי כמהנדס אזרחי בטכניון עם תשוקה עמוקה לציור וכתיבה.",
  },
  {
    icon: BookOpen,
    ...
    <main className="px-4 pt-2 pb-8">
      <div className="mx-auto max-w-[1400px]">
        ...
        <div className="relative flex flex-col md:flex-row items-stretch md:items-start justify-between gap-6" dir="ltr">
          ...
          <div className="order-2 md:order-none grid md:hidden grid-cols-3 gap-3 pt-2">
            {galleryFrameItems.map((it, i) => (
              <GalleryFrame
                key={`m-${it.key}`}
          ...
          <article
            className="order-1 md:order-none relative z-10 overflow-hidden rounded-[2rem] border border-[#d4c4a7] bg-[#f8f1e5] shadow-[0_24px_70px_rgba(66,44,20,0.16)] shrink-0 w-full md:w-[980px] md:[zoom:0.48]"
          >
...
const StorePage = () => {
  ...
  return (
    <div
      ...
      style={store.id === "s15" ? { fontFamily: '"Amatic SC", "Noto Sans Hebrew", cursive', color: '#000', fontWeight: 700 } : undefined}
    >
      <MallHeader />
      <PageTracker storeId={store.id} />
      <BackButton />
                ...
                <div className="grid grid-cols-1 md:grid-cols-[3fr_7fr] gap-4 items-stretch">
                  ...
                  <div className="rounded-lg bg-muted border border-border p-5 text-right">
                    ...
                    <p className="text-sm md:text-base text-muted-foreground font-heebo leading-relaxed">
                      בחורה שכולה מתנה אחת גדולה לכם ולכל מי שפתוח למה שיש לה להציע, להשתתף בפעילות חברתית, ללמוד מכל אחד אשר יוזם ומעניק אפשרות להעשיר לך את הידע בזווית ראייה יחודית או הרצון בעולם המופלא של חיבור לרגש וצורך של כולנו לחוות, טולי מציעה שלל של מפגשים וסדנאות אשר יכולים להיות הערב הפעיל שלכם, מלבד זה היא היוצרת של כלים וחומרים שכולם הם למטרה אחת, לתת לכם אהבה ואהבה לעצמכם, טולי מציגה את המוצרים שלה בירידי אומנים ששווה להתעדכן ולהשתתף וזאת הזדמנות טובה להכיר אדם שקסם לי מהרגע הראשון והרצון הטוב שלה לתרום כמה שיותר עם מוצרים רוחניים ופעילויות חברתיות.
                    </p>
                  </div>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-[3fr_7fr] gap-4 items-stretch">
                   ...
                   <div className="grid grid-cols-2 md:grid-cols-4 gap-3 h-full">
                     {(() => {
                      ...
                      <button
                        ...
                        className="rounded-lg overflow-hidden h-full flex flex-col text-right hover:ring-2 hover:ring-mall-gold transition"
                        style={{ backgroundColor: '#f4e8cc' }}
                        aria-label={`בחר מוצר ${p.name}`}
                      >
                        <img src={p.src} alt={p.name} className="w-full h-1/2 object-cover block" loading="lazy" />
              ...
              (() => {
                ...
                const ProductCard = ({
                  ...
                  <div className="flex flex-col items-center w-full">
                    ...
                    {/* Realistic solid wood shelf */}
                    <div className="w-[110%] -mx-[5%] mt-1">
                      <div
                        className="relative h-6 md:h-7 rounded-[3px] overflow-hidden"
                        style={{
                          background:
                            "linear-gradient(180deg, #d9a35f 0%, #b87a3a 35%, #8a5421 78%, #5e3713 100%)",
                          boxShadow:
                            "inset 0 1px 0 rgba(255,235,200,0.85), inset 0 -2px 4px rgba(40,20,5,0.55), 0 12px 18px -6px rgba(40,20,5,0.55)",
                        }}
                      >
                        {/* organic flowing wood grain */}
                        <div
                          className="absolute inset-0 opacity-60 mix-blend-multiply"
                          style={{
                            backgroundImage:
                              "radial-gradient(ellipse 120% 18% at 30% 35%, rgba(70,38,12,0.55), transparent 60%), radial-gradient(ellipse 90% 12% at 70% 70%, rgba(60,30,8,0.45), transparent 65%), radial-gradient(ellipse 60% 8% at 15% 80%, rgba(80,45,15,0.4), transparent 70%), radial-gradient(ellipse 140% 10% at 50% 20%, rgba(255,220,170,0.35), transparent 60%)",
                          }}
                        />
                        {/* natural knot */}
                        <div
                          className="absolute top-[55%] left-[22%] -translate-y-1/2 w-3 h-2 rounded-full opacity-75"
                          style={{
                            background:
                              "radial-gradient(ellipse at 40% 40%, #2e1808 0%, #5a3413 50%, transparent 85%)",
                          }}
                        />
                        {/* top satin highlight */}
                        <div
                          ...
                          style={{
                            background:
                              "linear-gradient(180deg, rgba(255,235,200,0.4), transparent)",
                          }}
                        />
                      </div>
                      {/* soft cast shadow */}
                      <div
                        className="mx-2 h-1.5 rounded-full opacity-50"
                        style={{
                          background:
                            "radial-gradient(ellipse at center, rgba(40,20,5,0.5), transparent 70%)",
                        }}
                      />
                    </div>
                  </div>
