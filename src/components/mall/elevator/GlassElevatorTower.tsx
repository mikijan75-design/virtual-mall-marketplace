...
const FloorLanding = ({
  ...
  <div className="absolute left-[-48%] right-[-72%] z-30 h-[9%]" style={{ top }}>
    ...
    <div
      className="absolute left-[8%] right-[4%] top-[4%] h-[48%] rounded-sm"
      style={{
        background:
    ...
    />
    <div className="absolute left-[8%] right-[4%] top-[1%] h-1.5 rounded-full bg-[linear-gradient(180deg,#f7ffff,#87a3aa_70%,#536970)]" />
    {[18, 42, 66, 88].map((left) => (
      <div
        key={left}
        className="absolute top-[3%] h-[52%] w-px bg-[linear-gradient(180deg,#ffffff,#77969d)]"
        style={{ left: `${left}%` }}
      />
    ))}
    {passenger === "blue" && (
...
const GlassElevatorTower = ({ side }: { side: "left" | "right" }) => (
  <div
    className={`pointer-events-none absolute top-[76px] bottom-[48px] z-[34] hidden w-[10.5%] md:block ${
